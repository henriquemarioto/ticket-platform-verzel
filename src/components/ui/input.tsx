    ...
    return (
      <div className="flex flex-col gap-1 w-full">
        <div className="relative">
          {icon && (
            <div className="absolute left-2 top-5/12 -translate-y-1/2 text-text-muted h-4 w-4">
              {icon}
            </div>
          )}
          <input
            type={type}
            className={cn(
              "flex h-10 w-full rounded-md shadow-sm bg-bg-surface px-3 py-2 text-sm text-text-primary transition-colors file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-text-muted/50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/50 disabled:cursor-not-allowed disabled:opacity-50",
              icon && "pl-10",
